...
function EditInstallmentModal({ contractId, installment, onClose, onSuccess }: EditInstallmentModalProps) {
  const [form, setForm] = useState({
    installment_no: String(installment.installment_no),
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="w-full max-w-lg bg-white rounded-lg shadow-xl">
        <ModalHeader title={`Taksit Düzenle — #${installment.installment_no}`} onClose={onClose} />
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Taksit No">
              <input type="number" required value={form.installment_no} onChange={e => set('installment_no', e.target.value)} className={inputCls} />
            </FormField>
            <FormField label="Ödeme Tipi">
              <select value={form.payment_type} onChange={e => set('payment_type', e.target.value)} className={inputCls}>
                {(Object.keys(PAYMENT_TYPE_LABELS) as PaymentType[]).map(t => (
          ...
          <FormField label="Açıklama">
            <input value={form.description} onChange={e => set('description', e.target.value)} className={inputCls} placeholder="İsteğe bağlı" />
          </FormField>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Tutar (₺) *">
              <input required type="number" step="0.01" value={form.amount} onChange={e => set('amount', e.target.value)} className={inputCls} />
            </FormField>
            <FormField label="Vade Tarihi *">
              <input required type="date" value={form.due_date} onChange={e => set('due_date', e.target.value)} className={inputCls} />
            </FormField>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <FormField label="Durum">
              <select value={form.status} onChange={e => set('status', e.target.value)} className={inputCls}>
                {(Object.keys(INSTALLMENT_STATUS_LABELS) as InstallmentStatus[]).map(s => (
                  <option key={s} value={s}>{INSTALLMENT_STATUS_LABELS[s]}</option>
                ))}
              </select>
            </FormField>
            <FormField label="Ödenen Tutar (₺)">
              <input type="number" step="0.01" value={form.paid_amount} onChange={e => set('paid_amount', e.target.value)} className={inputCls} />
            </FormField>
          </div>
          <FormField label="Ödeme Tarihi">
          ...
          <ModalFooter onClose={onClose} saving={saving} submitLabel="Güncelle" />
        </form>
      </div>
    </ModalOverlay>
  );
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {editInstallment && (
        <EditInstallmentModal
          ...
          onSuccess={() => { setEditInstallment(null); onRefresh(); }}
        />
      )}
    </div>
  );
