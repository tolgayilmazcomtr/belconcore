...
export default function PayableReport() {
    ...
    useEffect(() => {
        ...
        api.get('/accounting/invoices', { params: { active_project_id: activeProject.id, type: 'purchase', per_page: 1000 } })
    ...
    return (
        <div className="p-6 space-y-6 max-w-5xl">
            ...
            <div className="grid grid-cols-4 gap-4">
                {[
                    { label: 'Toplam Borç', value: fmt(stats.total), color: 'text-red-600', bg: 'bg-red-50' },
                    { label: 'Vadesi Geçmiş', value: fmt(stats.overdue), color: 'text-red-700', bg: 'bg-red-100' },
            ...
            {loading ? <p className="text-slate-400 text-sm">Yükleniyor...</p> : invoices.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-slate-400 text-sm">Ödenmemiş borç bulunmuyor.</p>
                </div>
            ) : (
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                        ...
                        return (
                            <div key={name}>
                                ...
                                {acctInvoices.map(inv => (
                                    <div key={inv.id} className={`grid grid-cols-[2fr_1fr_1fr_1fr_1fr] px-4 py-2.5 border-b border-slate-50 text-sm hover:bg-slate-50 ${isOverdue(inv.due_date) ? 'bg-red-50/30' : ''}`}>
                                        ...
                                        <span className={`text-xs font-mono font-semibold text-right ${isOverdue(inv.due_date) ? 'text-red-600' : 'text-red-500'}`}>{fmt(inv.remaining)}</span>
                                    </div>
                                ))}
                            </div>
                        );
                    ...
                    <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] px-4 py-3 bg-slate-900 text-white text-sm font-bold">
                        <span>TOPLAM BORÇ</span><span /><span /><span />
                        <span className="text-right font-mono text-red-400">{fmt(stats.total)}</span>
                    </div>
                </div>
            )}
        </div>
