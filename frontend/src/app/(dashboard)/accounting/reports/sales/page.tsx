...
export default function SalesReport() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const params: any = { active_project_id: activeProject.id, type: 'sales', per_page: 1000, date_from: `${year}-01-01`, date_to: `${year}-12-31` };
    ...
    return (
        <div className="p-6 space-y-6 max-w-5xl">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-base font-semibold text-slate-800 flex items-center gap-2"><ShoppingCart className="w-4 h-4 text-blue-600" />Satış Raporu</h1>
            ...
            <div className="grid grid-cols-5 gap-4">
                {[
                    { label: 'Toplam Satış', value: fmt(stats.total), color: 'text-blue-700', bg: 'bg-blue-50' },
                    { label: 'Tahsil Edilen', value: fmt(stats.paid), color: 'text-green-600', bg: 'bg-green-50' },
                    { label: 'Tahsil Edilecek', value: fmt(stats.remaining), color: 'text-amber-600', bg: 'bg-amber-50' },
            ...
            {loading ? <p className="text-slate-400 text-sm">Yükleniyor...</p> : invoices.length === 0 ? (
                <div className="py-16 text-center"><p className="text-slate-400 text-sm">Seçili dönemde satış faturası bulunamadı.</p></div>
            ) : (
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                    <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] px-4 py-2.5 bg-slate-50 border-b text-[10px] font-semibold text-slate-500 uppercase tracking-wide">
                        <span>Fatura</span><span className="text-center">Tarih</span><span className="text-center">Cari</span>
                    ...
                    {invoices.map(inv => (
                        <div key={inv.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] px-4 py-2.5 border-b border-slate-50 hover:bg-slate-50 transition-colors">
                            ...
                            <span className={`text-xs font-mono text-right ${inv.remaining > 0 ? 'text-amber-600' : 'text-slate-400'}`}>{inv.remaining > 0 ? fmt(inv.remaining) : '—'}</span>
                    ...
                    <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] px-4 py-3 bg-slate-900 text-white text-sm font-bold">
                        ...
                        <span className="text-right font-mono text-amber-400">{fmt(stats.remaining)}</span>
                        <span />
                    </div>
                </div>
            )}
